...
import type { ContentPage, ParsedWikiLink, ResolveContext, ResolvedWikiLink, WikiSubpath } from "./types.js";
...
export function resolveWikiLink(
  ...
  const exactBaseNameResolution = resolveCandidateSet(exactBaseNameCandidates, parsed.target, "path-qualified link");
  ...
  const metadataResolution = resolveCandidateSet(metadataCandidates, parsed.target, "a more specific filename");
...
function resolveFuzzyPage(
  ...
  const suffixMatches = context.index.pages.filter((page) => {
    ...
    return normalizedPagePath === normalizedTarget || normalizedPagePath.endsWith(`/${normalizedTarget}`);
  });
  ...
  const sortedMatches = [...suffixMatches].sort((left, right) => left.pathKey.length - right.pathKey.length);
